...
from .events import ROOMS  # Importa a lista de salas de chat do events.py
...
def lobby():
    """
    Renderiza a página inicial (lobby)
    """
...
def rooms(username):
    """
    Renderiza a página de menu de salas para um usuário específico
    """
    if username:
        return render_template("rooms.html", username=username)
    else:
        # Se faltar o nome de usuário, volta para o lobby
...
def chat(room_name, username):
    ...
    if room_name in ROOMS and username:
        ...
        # Se faltar informação ou se a sala não existir, volta para o Rooms
...
def code(room_name, username):
    """
    Renderiza a página de solicitação de senha para uma sala privada e usuário específicos
    """
    if room_name in ROOMS and username:
